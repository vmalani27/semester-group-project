...
def get_plain_text(payload):
    """Extract plain text content from a message payload, handling both text/plain and text/html."""
    try:
        # Use BeautifulSoup to parse HTML content and extract text
        return bs4.BeautifulSoup(payload, 'lxml').get_text(' ', strip=True)
    except Exception as e:
        # In case of parsing issues, return the payload as a string
        return str(payload)
...
class GmailMboxMessage:
    ...
    def parse_email(self):
        # Extract relevant email headers and content
        email_labels = self.email_data.get('X-Gmail-Labels', '')
        email_date = self.email_data.get('Date', '')
        email_from = self.email_data.get('From', '')
        email_to = self.email_data.get('To', '')
    ...
    def _extract_text_content(self, email_messages):
        ...
        for msg in email_messages:
            ...
            if content_type in ('text/plain', 'text/html'):
                # Decode and clean the message payload
                msg_text = get_plain_text(msg.get_payload(decode=True).decode('utf-8', errors='ignore'))
                if msg_text:
